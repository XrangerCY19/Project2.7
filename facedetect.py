...
if __name__ == '__main__':
    ...
    parser.add_argument('-v', '--video', dest='source', help='Path to video')   #command line argument to pass video location
    args = parser.parse_args()
    source = args.source

    if (args.source):
        if (args.source == '0'):    #for webcam, we convert string to int else cvtColor error
            source = int('0')
